...
@router.get(
    "/all", response_description="Get all users", response_model=list[User]
)
...
@router.post(
    "/create", response_description="Create a new user", response_model=User
)
async def create_user(
    user_create: UserBase, session: Session = Depends(get_session)
):
