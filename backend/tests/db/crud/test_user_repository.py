...
def test_find_by_emails(session):
    ...
    # when
    users = user_repository.find_by_emails(["test1@example.com", "test2@example.com"], session)
...
def test_find_with_public_keys_by_emails(session):
    ...
    # when
    users = user_repository.find_with_public_keys_by_emails(["test1@example.com", "test2@example.com"], session)
...
def test_all_exist(session):
    ...
    assert not user_repository.all_exist(["test1@example.com", "test3@example.com"], session)
...
def test_all_exist_and_have_public_keys(session):
    ...
    # when & then
    assert user_repository.all_exist_and_have_public_keys(["test1@example.com", "test2@example.com"], session)
    assert not user_repository.all_exist_and_have_public_keys(["test1@example.com", "test3@example.com"], session)
...
def test_all_have_public_keys(session):
    ...
    # when & then
    assert user_repository.all_have_public_keys([1, 2], session)
    assert not user_repository.all_have_public_keys([1, 3], session)
